#!/usr/bin/env python3
"""
Manual Review of SmolVLM-2.2B Hallucination Dataset
...
def semantic_review(question, ground_truth, model_answer):
    ...
    no_patterns = ['no ', 'zero ', 'without ', 'none', 'not ', "isn't", "aren't", "doesn't", "don't see", 'not depicted', 'does not show', 'not wearing']
    ...
    if gt_has_no and model_has_no:
        ...
        if 'hat' in question_lower:
            if 'no hat' in model_lower or 'not wearing' in model_lower or "doesn't have" in model_lower or "does not show" in model_lower or 'does not contain' in model_lower:
    ...
    if 'does not have' in gt_lower or 'without' in gt_lower or 'is not' in gt_lower:
        ...
        if 'hat' in gt_lower and 'hat' in question_lower:
            ...
            if 'no hat' in model_lower or "doesn't have" in model_lower or "does not show" in model_lower or 'not wearing' in model_lower or 'does not contain' in model_lower:
    ...
    stopwords = {'the', 'a', 'an', 'is', 'are', 'was', 'were', 'in', 'on', 'at', 'to', 'of', 'and', 'or', 'i', 'you', 've', 'seems', 'appear', 'appears'}
...
# Main processing
print("="*80)
print("SMOLVLM-2.2B MANUAL REVIEW - LENIENT HALLUCINATION FLAGGING")
print("="*80)
print()

# Load dataset
input_file = Path('smolvlm_hallucination_dataset.csv')
output_file = Path('smolvlm_manually_reviewed.csv')
...
df['is_hallucinating_manual'] = reviewed_flags  # Add manual column for consistency
...
print()
print("="*80)
print("MANUAL REVIEW COMPLETE - SMOLVLM-2.2B")
print("="*80)
print(f"\nTotal entries: {total:,}")
...
print()
print("✅ SMOLVLM-2.2B MANUAL REVIEW COMPLETE")
